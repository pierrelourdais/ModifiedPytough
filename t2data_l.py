    ...
    def run(self,save_filename='',incon_filename='',simulator='AUTOUGH2_2',silent=False):
        ...
        if self.filename:
            ...
            if (self.type=='AUTOUGH2'):
                ...
                # run AUTOUGH2:
                #debug
                print('./'+simulator+' < '+runfilename+out)
...
#             outfile.write('\n')
    ...
    def write_incons(self,outfile):
        if self.incon:
            outfile.write('INCON\n')
            i=0
            for blk in self.incon.blocklist:
                vals=[unfix_blockname(blk),self.incon.porosity[i]]
                outfile.write_values(vals,'incon1')
                a=self.incon.variable[i]
                outfile.write_values(a,'incon2')
                i+=1
            outfile.write('\n')
