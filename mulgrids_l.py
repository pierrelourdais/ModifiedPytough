...
""" Modification Lourdais: commented 1056 """
...
class mulgrid(object):
    ...
    def create_cylinder(self,hmin,length,radii,theta_step,layer_step,radii_step,nameofcylinder,filename):
        import math
        name_list=range(10)+map(chr, range(97, 123))+map(chr, range(65, 91))
        ###############
        #Build Layers
        ###############
        for i in range(layer_step):
            if i<10:name_layer='0'+str(i)
            else:name_layer=str(i)
            lay=layer(name=name_layer,bottom=hmin+i*(length/layer_step),centre=hmin+(i+.5)*(length/layer_step),top=hmin+(i+1)*(length/layer_step))
            self.add_layer(lay)
        ...
        ###############
        #Initialiqation
        n=node(name='0'+nameofcylinder+'0', pos=[0,0])
        self.add_node(n)
        
        #Loop
        for j in range(1,radii_step+1):
            for k in range(theta_step):
                
                name=str(name_list[j])+nameofcylinder+str(name_list[k])
                x=radii*(float(j)/(radii_step+1))*math.cos(2*math.pi*float(k)/theta_step)
                y=radii*(float(j)/(radii_step+1))*math.sin(2*math.pi*float(k)/theta_step)
                pos=np.array([x,y])
                n=node(name=name,pos=pos)
                self.add_node(n)
        
        ###############
        #Build Column
        ###############
        permutation=range(theta_step)+[0]
        
        #Initialisation
        for j in range(theta_step):
            list_col=[self.node['0'+nameofcylinder+'0'],self.node['1'+nameofcylinder+str(name_list[permutation[j]])], self.node['1'+nameofcylinder+str(name_list[permutation[j+1]])]]
            col=column(name='0'+nameofcylinder+str(name_list[j]),node=list_col,surface=hmin+length)
            self.add_column(col)
        
        
        #Loop
        for j in range(1,radii_step):
            for k in range(theta_step):
                nodelist=[]
                name=str(name_list[j])+nameofcylinder+str(name_list[k])
                nodelist.append(self.node[name])
                name=str(name_list[j])+nameofcylinder+str(name_list[permutation[k+1]])
                nodelist.append(self.node[name])
                name=str(name_list[j+1])+nameofcylinder+str(name_list[k])
                nodelist.append(self.node[name])
                name=str(name_list[j+1])+nameofcylinder+str(name_list[permutation[k+1]])
                nodelist.append(self.node[name])
                
                col=column(name=str(name_list[j])+nameofcylinder+str(name_list[k]),node=nodelist,surface=length)
                self.add_column(col)
        ...
        permutation=range(theta_step)+[0]
        for j in range(theta_step):
            name1='0'+nameofcylinder+str(name_list[permutation[j]])
            name2='0'+nameofcylinder+str(name_list[permutation[j+1]])
            name3='1'+nameofcylinder+str(name_list[permutation[j]])
            con2=connection([self.column[name1],self.column[name3]])
            con1=connection([self.column[name1],self.column[name2]])
        ...
        for j in range(1,radii_step-1):
            for k in range(theta_step):
                name1=str(name_list[j])+nameofcylinder+str(name_list[permutation[k]])
                name2=str(name_list[j])+nameofcylinder+str(name_list[permutation[k+1]])
                name3=str(name_list[j+1])+nameofcylinder+str(name_list[permutation[k]])
                con2=connection([self.column[name1],self.column[name3]])
                con1=connection([self.column[name1],self.column[name2]])
                self.add_connection(con1)
                self.add_connection(con2)
        ...
        for k in range(theta_step):
            ...
            name1=str(name_list[radii_step-1])+nameofcylinder+str(name_list[permutation[k]])
            name2=str(name_list[radii_step-1])+nameofcylinder+str(name_list[permutation[k+1]])
            con=connection([self.column[name1],self.column[name2]])
            self.add_connection(con)
            
        ###################
        #Set layers in each column
        ###################
    ...
    def substract_cylinder(self,radii,theta_step):
        import math
        deleted_column=[]
        
        for compteur in range(theta_step):
            for col in self.columnlist:
                radii_col=math.sqrt(col.centre[0]**2+col.centre[1]**2)
                if radii_col<radii:
                    deleted_column.append(col.name)
