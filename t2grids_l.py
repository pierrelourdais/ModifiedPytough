...
class rocktype(object):
    """Rock type"""
    def __init__(self,name="dfalt",nad=0,density=2600.0,porosity=0.1,permeability=1.0e-15*np.ones(3),conductivity=1.5,specific_heat=900.0,compressibility=0.0):
        self.name=name
        self.nad=nad
...
class t2grid(object):
    ...
    #Pierre
    def create_cylinder_connection(self,smaller_grid,larger_grid,radii_step_int,layer_step_int,radii_step_ext,layer_step_ext,theta_step):
        #enable to connect two cylinder grids
        #smaller grid is the smaller one (inside)
        #larger grid is the larger one (around)
        #self is the whole one
        centre_max=0
        list_max=[]
        import math
        self.get_block_centres_defined()
        smaller_grid.get_block_centres_defined()
        larger_grid.get_block_centres_defined()
        
        for blk in smaller_grid.blocklist[1:]:
            radii=math.sqrt(blk.centre[0]**2+blk.centre[1]**2)
            if radii>centre_max:
                centre_max=radii
                list_max=[blk]
            elif radii==centre_max: list_max.append(blk)
        
        centre_min=math.sqrt(larger_grid.blocklist[1].centre[0]**2+larger_grid.blocklist[1].centre[1]**2)
        list_min=[]
        for blk in larger_grid.blocklist[1:]:
            radii=math.sqrt(blk.centre[0]**2+blk.centre[1]**2)
            if radii<centre_min:
                centre_min=radii
                list_min=[blk]
            elif radii==centre_min: list_min.append(blk)
            
        for blk_int in list_max:
            for blk_ext in list_min:
                diff_elevation_square=0
                distance=0
                for i in range(2):distance+=(blk_ext.centre[i]-blk_int.centre[i])**2
                diff_elevation_square=distance- (0.5*(radii_step_ext+radii_step_int))**2
                
                diff_elevation=math.sqrt(diff_elevation_square)
                h=0.5*(layer_step_int-layer_step_ext)-diff_elevation
                if h>0:
                    area=h*math.sqrt(blk_int.centre[0]**2+blk_int.centre[1]**2)*(1.0/theta_step)
                    con=t2connection(blocks=[blk_ext,blk_int],direction=1,distance=[0.0,0.0],area=area,dircos=0.0)
                    self.add_connection(con)
                    
                    
                
    def create_vertical_connection(self,upper_grid,radii_step_up,lower_grid,radii_step_low,theta_step,rayon):
        elevation_max=0
        list_max=[]
        import math
        for blk in lower_grid.blocklist[1:]:
            radii=math.sqrt(blk.centre[0]**2+blk.centre[1]**2)
            if blk.centre[2]>elevation_max:
                elevation_max=blk.centre[2]
                list_max=[blk]
            elif blk.centre[2]==elevation_max and radii<rayon:
                list_max.append(blk)
        
        elevation_min=upper_grid.blocklist[1].centre[2]
        list_min=[]
        for blk in upper_grid.blocklist[1:]:
            radii=math.sqrt(blk.centre[0]**2+blk.centre[1]**2)
            if blk.centre[2]<elevation_min:
                elevation_min=blk.centre[2]
                list_min=[blk]
            elif blk.centre[2]==elevation_min and radii<rayon:
                list_min.append(blk)
            
        for blk_low in list_max:
            for blk_up in list_min:
                distance= math.sqrt((blk_up.centre[0]-blk_low.centre[0])**2+(blk_up.centre[1]-blk_low.centre[1])**2)   
                radii_up=math.sqrt(blk_up.centre[0]**2+blk_up.centre[1]**2)
                radii_low=math.sqrt(blk_low.centre[0]**2+blk_low.centre[1]**2)
                angle_up=math.acos(blk_up.centre[0]/radii_up)
                angle_low=math.acos(blk_low.centre[0]/radii_low)
                h=0.5*(radii_step_up+radii_step_low)-distance
                area= math.fabs(((0.5*radii_step_up)+radii_up)**2-((0.5*radii_step_low)+radii_low)**2)*math.pi/theta_step
                if h>0 and math.fabs(angle_low-angle_up)<2*math.pi/theta_step:
                    con=t2connection(blocks=[blk_up,blk_low],direction=1,distance=[0.0,0.0],area=area,dircos=0.0)
                    self.add_connection(con)
    ...
    def add_horizontal_layer_connections(self,geo,lay,layercols=[]):
        ...
        for con in [con for con in geo.connectionlist if set(con.column).issubset(layercolset)]:
            ...
            if np.linalg.norm(d)==0: print con
